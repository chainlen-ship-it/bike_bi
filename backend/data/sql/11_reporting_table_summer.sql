-- 11_reporting_table_summer.sql
--
-- Course 2 summer reporting target table. Same shape as
-- reporting_full_year, filtered to July, August, and September only,
-- to support summer-focused dashboard views and the Course 1
-- "Summer 2015" requirement.
--
...
--
-- Output supports:
--   * Summer-trend chart on the Course 2 dashboard page
--   * Borough-by-borough comparison of summer ridership
--   * Subscriber vs casual summer behavior, joined to weather
--
-- Notes:
--   * Years filtered to 2014-2015 per the course query.
--   * Months filtered to 7, 8, 9 (July, August, September).
--   * DATE_ADD shifts dates +5 years so the dashboard appears recent.
--   * Replace YOUR_PROJECT with your GCP project ID, and adjust the
--     cyclistic.zip_codes reference if you uploaded the CSV under a
--     different dataset/table name.

CREATE OR REPLACE TABLE `YOUR_PROJECT.cyclistic_bi.reporting_summer` AS
SELECT
  TRI.usertype,
...
WHERE
  ...
  AND EXTRACT(MONTH FROM DATE(TRI.starttime)) BETWEEN 7 AND 9
GROUP BY
